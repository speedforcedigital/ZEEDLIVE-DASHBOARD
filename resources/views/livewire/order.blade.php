<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <li class="m-1">
        <button wire:click="filter('pending')"
            ...
            Pending <span class="ml-1 {{ $selected === 'pending' ? 'text-indigo-200' : 'text-slate-400' }}">{{ $totalPendingOrders }}</span>
        </button>
    </li>
    <li class="m-1">
        <button wire:click="filter('shipped')"
            ...
            Shipped <span class="ml-1 {{ $selected === 'shipped' ? 'text-indigo-200' : 'text-slate-400' }}">{{ $totalShippedOrders }}</span>
        </button>
    </li>
    <li class="m-1">
        <button wire:click="filter('delivered')"
            ...
            Delivered <span class="ml-1 {{ $selected === 'delivered' ? 'text-indigo-200' : 'text-slate-400' }}">{{ $totalDeliveredOrders }}</span>
        </button>
    </li>
</ul>
                        ...
                        <tr>
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="font-medium text-slate-800"> <a href="{{route('user.show',$order->seller->id)}}"> {{$order->seller->name}}</a></div>
                                </div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="font-medium text-slate-800"><a href="{{route('user.show',$order->customer->id)}}">{{$order->customer->name}} </a> </div>
                                </div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="flex items-center">
    ...
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <!-- Modal content -->
        <div class="bg-white rounded-xl w-9/10 sm:w-3/4 md:w-1/2 lg:w-1/3 xl:w-1/4 overflow-hidden shadow-2xl transform transition-all">
            <!-- Header -->
            ...
            <!-- Body -->
        <div class="p-5">
            <p class="text-gray-700 leading-relaxed"><strong>Seller:</strong>  <a href="{{ route("user.show",$order->customer->id ) }}">  {{ $order->customer->name }}</a>
            </p>
            <p class="text-gray-700 leading-relaxed"><strong>Product:</strong> <a href="{{ route("product.show",$order->lot->id ) }}">  {{ $order->lot->title }}</a>
            </p>
        ...
        {{-- @endforeach --}}
            ...
            <div class="my-4">
                ...
                <ul>
                    ...
                    <li><a href="{{ $order->seller->SellerVerification->document1 }}" class="text-blue-500 hover:underline"
