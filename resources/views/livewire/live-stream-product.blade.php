<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
    ...
    <li class="m-1">
        <button wire:click="filter('all')"
            ...
            All <span class="ml-1 {{ $selected === 'all' ? 'text-indigo-200' : 'text-slate-400' }}">{{ $total_products_count }}</span>
        </button>
    </li>
    <li class="m-1">
        <button wire:click="filter('scheduled')"
            ...
            Schdeuled <span class="ml-1 {{ $selected === 'scheduled' ? 'text-indigo-200' : 'text-slate-400' }}">{{ $scheduledProducts }}</span>
        </button>
    </li>
    <li class="m-1">
        <button wire:click="filter('on_going')"
            ...
            On Going <span class="ml-1 {{ $selected === 'on_going' ? 'text-indigo-200' : 'text-slate-400' }}">{{ $onGoingProducts }}</span>
        </button>
    </li>
    <li class="m-1">
        <button wire:click="filter('ended')"
            ...
            Closed <span class="ml-1 {{ $selected === 'ended' ? 'text-indigo-200' : 'text-slate-400' }}">{{ $endedProducts }}</span>
        </button>
    </li>
    <li class="m-1">
        <button wire:click="filter('sold')"
            ...
            Sold <span class="ml-1 {{ $selected === 'sold' ? 'text-indigo-200' : 'text-slate-400' }}">{{ $sold_products }}</span>
        </button>
    </li>
</ul>
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                ...
                                @if($product->auction->auction_status == "Open")
                                     <button class="text-slate-400 hover:text-slate-500 rounded-full"
                                        wire:click="view({{ $product->id }})">
                                        <span class="sr-only">View</span>
                                       <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-device-tv" width="27" height="27" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                    ...
                                    <button class="text-slate-400 hover:text-slate-500 rounded-full"
                                       wire:click="view({{ $product->id }})">
                                       <span class="sr-only">View</span>
                              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-player-stop" width="27" height="27" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50" fill="none" stroke-linecap="round" stroke-linejoin="round">
  <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
